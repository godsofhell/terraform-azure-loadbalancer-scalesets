...
resource "azurerm_subnet" "network_subnets" {
    ...
    //to be improved with dynamic calculation of address prefixes
    //create different subnets based on count index
    // 8 bits for each subnet
...
resource "azurerm_network_interface" "network_interfaces" {
  ...
  name                = "networkinterface0${count.index + 1}"
  location            = var.location
  resource_group_name = var.resource_group_name
